			...
			<div class="new-form-container">
				...
				<ul role="tablist" class="nav nav-tabs">
					<li class="nav-item"><a href="#investment" role="tab" data-toggle="tab" class="nav-link active"
											aria-selected="true"><i class="fal fa-cart-plus mr-2"></i> Purchase Pack</a>
					</li>
					...
					<div role="tabpanel" id="investment" class="tab-pane fade in active show">
						<form autocomplete="off" method="POST" action="{{ route('ad_packs.post') }}" id="adPackPurchaseForm">
							...
							<div class="row">
									...
									<div class="form-group"><label for="sel2">Purchase:</label>
										<select class="custom-select" name="number" id="number">
											...
											@if (current_user()->membership()->id === 1)
												...
												@endfor
											@elseif (current_user()->membership()->id === 2)
												@for ($i = 1; $i <= 20; $i++)
													<option value="{{ $i }}">No of AdPack {{ $i }}</option>
												@endfor
											@elseif (current_user()->membership()->id === 3)
												@for ($i = 1; $i <= 50; $i++)
													<option value="{{ $i }}">No of AdPack {{ $i }}</option>
												@endfor
											@elseif (current_user()->membership()->id === 4)
												@for ($i = 1; $i <= 100; $i++)
													<option value="{{ $i }}">No of AdPack {{ $i }}</option>
												@endfor
											@elseif (current_user()->membership()->id === 5)
												@for ($i = 1; $i <= 200; $i++)
													<option value="{{ $i }}">No of AdPack {{ $i }}</option>
												@endfor
											@elseif (current_user()->membership()->id === 6)
												@for ($i = 1; $i <= 400; $i++)
													<option value="{{ $i }}">No of AdPack {{ $i }}</option>
												@endfor
											@elseif (current_user()->membership()->id === 7)
												@for ($i = 1; $i <= 2000; $i++)
													<option value="{{ $i }}">No of AdPack {{ $i }}</option>
												@endfor
											@elseif (current_user()->membership()->id === 8)
												@for ($i = 1; $i <= 4000; $i++)
													<option value="{{ $i }}">No of AdPack {{ $i }}</option>
												@endfor
											@endif
								...
								<div class="col-sm-12 col-md-6 col-lg-3 col-xl-3 op-6">
